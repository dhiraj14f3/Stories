...
BEGIN
    ...
    -- Reuse existing relation_identifier/master_id if present
    ...
    IF case_relation_id IS NULL THEN
        SELECT id
          INTO case_relation_id
          FROM d_relationships
    ...
    WHERE NOT EXISTS (
        ...
        WHERE df.d_entity_id = v_parent_entity_id
          AND df.field_name = 'case'
    )
    RETURNING id INTO case_field_id;

    IF case_field_id IS NULL THEN
        SELECT id
          INTO case_field_id
    ...
    WHERE df.d_entity_id = v_target_entity_id
      AND df.field_name = 'case_number'
      AND NOT EXISTS (
          SELECT 1
          FROM d_fields_relationship fr
          WHERE fr.parent_field_id = case_field_id
