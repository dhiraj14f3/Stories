...
CREATE OR REPLACE FUNCTION "fpa-1003".link_triage_document(
    p_case_id BIGINT,
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1
        FROM "fpa-1003".c_case_documents
    ...
    --------------------------------------------------
    IF v_exists THEN
        UPDATE "fpa-1003".c_case_documents
        SET document_id = p_document_id
        WHERE case_id = p_case_id
          AND name = v_name;
    ELSE
        INSERT INTO "fpa-1003".c_case_documents (case_id, document_id, name)
